...
- Use a single owner unless acceptance criteria map to files or modules with no shared edits; only then fan out, recording the disjoint mapping — and any other delegation rationale — whenever sub-agents are used.
- Preserve active WannaBuild workflow state across turns until the task is complete or the user explicitly exits or stops.
- Do not revert to generic implementation behavior; use the WannaBuild implementation contract.
...
4. Verify the slice. You MUST run, in fixed order, every check that applies and ALL must pass before continuing: (a) lint, (b) type/build, (c) the full relevant test suite, (d) the change exercised against a real running instance — start the app, hit the endpoint, or drive the UI in a real browser. There is no narrow-only stopping point, and a "manual check" never substitutes for any of these. Scope adapts to the diff (more tests on a larger change), but which checks run does not — two runs of the same task verify identically.
