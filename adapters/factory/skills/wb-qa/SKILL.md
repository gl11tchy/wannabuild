...
- Work in the current checkout by default; do not create an isolated worktree for QA.
...
The integration test gate is owned by `wb-integration-tester` and is the terminal hard gate. It PASSES only when all of the following hold and are evidenced:
...
1. Confirm the Preconditions. If any is missing, route back rather than guessing.
2. Build a complete coverage matrix mapping EVERY acceptance criterion and EVERY changed surface to the check(s) that verify it. There is no smallest-set, impacted-only, fast-track, or low-risk reduction of the check set.
...
4. For each criterion, execute the strongest applicable verification: run the automated test suite AND exercise the live end-to-end flow when the change touches a UI/service/data path. Inspection alone never passes a criterion that is executable. Capture real commands, exit codes, and output.
5. Run the integration hard gate (`wb-integration-tester`) and confirm its PASS criteria above are met.
6. If a genuine blocker remains after acquisition is exhausted, bring it to the user as a single decision with options and a recommended answer (see Resource Acquisition); never silently skip or mark "out of scope".
