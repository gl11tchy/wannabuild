...
2. **Identify implementation units:** Each task should be a single, focused change that can be committed independently.
...
6. **Size each task:** S (< 1 hour), M (1-3 hours), L (3+ hours). If L, consider breaking it down further.
...
- Tasks must be atomic: one task = one logical change = one commit.
