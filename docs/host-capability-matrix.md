...
- host mapping from core capability tiers to current Claude models via agent frontmatter: strong → Fable 5 (`claude-fable-5`), standard → Opus 4.8 (`opus`), lightweight → Haiku 4.5 (`haiku`); see `adapters/claude-code/README.md`
...
| Capability tier mapping | Tier/effort contract | Host mapped | Prompt/report fallback | Agent frontmatter: `claude-fable-5`/`opus`/`haiku` |
