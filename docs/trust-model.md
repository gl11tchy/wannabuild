...
| **Checked** | A program inspects artifacts and exits non-zero (`wb-runtime` gates, validators, hook mirrors) | Not by arguing — only by fabricating artifacts, which the checks are built to catch |
...
|---|---|---|
| Claude Code | Plugin hook injects live runtime state and gate results each turn; Python mirrors of every gate run even without the binary | `wb-runtime` if present, else the hook's `record-test-evidence` subcommand (same signed format) |
...
- **Fixture mode.** `WB_EVIDENCE_MODE=fixture` skips evidence verification so
  ...
  is labeled `FIXTURE MODE` in gate output and the event log. If you see that
  label in a run that claims to be real, it isn't.
