...
You also shouldn't need complete requirements up front. WannaBuild's first job is to interview well: draw out the vision, flow, feel, features, constraints, non-goals, and success signal before turning that into a spec.
...
Each expert does one thing exceptionally well, but the orchestrator should only use specialists when they add signal. No phase should become a committee just because specialists exist.
...
Code doesn't leave the loop until the active reviewer set agrees it's ready. The integration tester is always active and ensures every acceptance criterion has a passing test. No hand-waving.
...
- Sometimes one coherent implementer is better than a crowd
- Sometimes parallel specialists are exactly what keeps quality high
...
After implementation, selected specialist agents review in parallel when their perspectives are relevant:
...
Each active agent votes PASS or FAIL.

If **any** active agent fails, feedback goes back to the implementer. The loop continues until **all active reviewers vote PASS**.
